...
# Check events
kubectl get events -n dclaw-water --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-water deployment/dclaw-water-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-water-db-credentials -n dclaw-water
   ```
